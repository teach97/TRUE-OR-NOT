...
def test_updates_stream_uses_existing_frontend_stage_names():
    ...
    assert asyncio.run(collect()) == ["extracting", "searching", "reading", "verifying"]
...
def test_provider_failure_stops_graph_without_fabricated_result():
    ...
    graph = build_workflow(extract=failing, search=subsequent, read=subsequent, verify=subsequent)
