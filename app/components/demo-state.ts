export type Claim = Readonly<{ id: string; quote: string; start: number; end: number; verdict: string | null; tone: string; summary: string; evidenceIds: readonly string[] }>;
...
export function createPreview(draft: { text: string; focus: string }): Preview {
  const claims = Array.from(draft.text.matchAll(/[^.!?\n]+[.!?]?/g)).filter(m => m[0].trim()).slice(0, 3).map((m, i) => {
    ...
    return Object.freeze({ id: `claim-${i + 1}`, quote, start, end: start + quote.length, verdict: null, tone: 'neutral', summary: '문장 단위로 나눈 후보입니다. 사실 여부와 문장 유형은 분석하지 않았습니다.', evidenceIds: Object.freeze([] as string[]) });
  });
