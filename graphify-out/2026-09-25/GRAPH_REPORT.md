...
## Corpus Check
- 124 files · ~102,442 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 23 file(s) not represented in the graph (top: .log 6, (none) 5, .css 5)
...
- **Why does `ProviderCallError` connect `ProviderCallError` to `synthesize_answer`, `classify_intent`, `search_sources`, `verification.py`, `LLMProvider`, `runtime.py`, `answer_synthesis.py`, `make_runtime_adapters`, `providers.py`, `main.py`, `request_structured`, `extraction.py`, `extract_claims`, `search.py`?**
